...
import de.hackerspacebremen.deprecated.data.entity.FindInterface;
...
public class JSONFormatter implements Formatter {
	...
	@Override
	public <T extends BasicEntity> T reformat(final String object,
			...
			try {
				...
				for (final String key : fieldMap.keySet()) {
					if (json.has(key)) {
						...
						if (FindInterface.class.isAssignableFrom(field
								.getType())) {
							final Method find = field.getType().getMethod(
									"find", new Class<?>[] { String.class });
							if (find != null) {
								final Object enumObject = find.invoke(
										Enum.class, json.get(key));
