 ...
 *
 */
public class ActivationEmail implements SendEmail{
	
	/**
	 * static attribute used for logging.
	 */
	private static final Logger logger = Logger
			.getLogger(ActivationEmail.class.getName());

	private final DoorKeyKeeper keeper;
	
	public ActivationEmail(final DoorKeyKeeper keeper){
		this.keeper = keeper;
	}
	...
	public void send() {
		...
		try {
			...
			msg.setSubject(
					PropertyHelper.getEmailPropertyValue("email.activation.subject"),
					Constants.UTF8);
			msg.setHeader("Content-Type", "text/plain; charset=utf-8");
			final MessageFormat form = new MessageFormat(
					PropertyHelper.getEmailPropertyValue("email.activation.content"));
